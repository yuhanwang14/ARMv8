...
uint32_t parse_two_operands_with_des(char *opcode, char **arguments) {
    ...
    bit_append(&result, opc, 2); // opc,pos 30-29
    if (is_literal(arguments[2])) {
        printf("pos2\n%s\n", arguments[2]);
        bit_append(&result, DPI_IMM_26_28, 3);
        bit_append(&result, DPI_ARITH_OPI, 3);
        bit_append(&result, parse_imm12(arguments[2], arguments[3]), 13);
        bit_append(&result, parse_register(arguments[1]), REGISTER_ADR_SIZE);
        bit_append(&result, parse_register(arguments[0]), REGISTER_ADR_SIZE);
    } else {
        bit_append(&result, DPI_REG_ARITH_LOGIC_25_28, 4);
        if (logicOp) {
        ...
        bit_append(&result, parsedShift[0], 2);
        bit_append(&result, negate, 1);
        bit_append(&result, parse_register(arguments[2]), REGISTER_ADR_SIZE);
        bit_append(&result, parsedShift[1], 6);
        bit_append(&result, parse_register(arguments[1]), REGISTER_ADR_SIZE);
        bit_append(&result, parse_register(arguments[0]), REGISTER_ADR_SIZE);
        free(parsedShift);
    }
...
uint32_t parse_multiply(char *opcode, char **arguments) {
    ...
    bit_append(&result, DPI_MUL_21_30, 10);
    bit_append(&result, parse_register(arguments[2]), REGISTER_ADR_SIZE);
    ...
    bit_append(&result, parse_register(arguments[3]), REGISTER_ADR_SIZE);
    bit_append(&result, parse_register(arguments[1]), REGISTER_ADR_SIZE);
    bit_append(&result, parse_register(arguments[0]), REGISTER_ADR_SIZE);
    return result;
}
